...
5. 如果脚本输出的是完整 Markdown，最终回复必须直接原样返回该 stdout，不要做摘要，不要改写措辞，不要补“二维码已生成”“请扫描上方二维码”这类说明，也不要再额外包 `<img>` HTML 标签，更不要改写成 `MEDIA:` 行。
...
## 输出要求
优先直接返回脚本在 `--format markdown` 下生成的完整 Markdown，并且整条 assistant 最终回复只包含这段 Markdown。
