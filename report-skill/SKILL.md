...
6. 会员人数/会员新增类问题，不要猜测 `/report/memberAddSummary`、`/report/memberAnalysis` 这类未在 skill 中确认过的路径；优先使用 `customerPeriodSummary` 这条已确认路径和它的 `flag/stime/etime/period` 参数格式。
