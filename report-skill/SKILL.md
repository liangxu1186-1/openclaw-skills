...
7. 线上请求参数名保持 `publicKey` 不变，但其值只允许来自三处：优先读取环境变量 `OPENCLAW_IDENTITY`，否则读取 `~/.openclaw/identity/device.json` 中的 `deviceId`，最后兜底读取 `/home/gem/workspace/agent/identity/device.json`；不要再从其他路径推导或兜底。
8. 当前已登记的接口分为两类：
   - 已实现：可以正常用于查询和回答
